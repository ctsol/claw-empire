  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 border-b border-slate-700 px-5 py-3">
        ...
        >
          1. {t({ ko: "", en: "Select Repo", ja: "リポ選択", zh: "选择仓库", ru: "Выбор репо" })}
        </button>
        ...
        >
          2. {t({ ko: "", en: "Branch", ja: "ブランチ", zh: "分支", ru: "Ветка" })}
        </button>
        <span className="text-slate-600">/</span>
        <button
          type="button"
        ...
        >
          3. {t({ ko: "", en: "Import", ja: "インポート", zh: "导入", ru: "Импорт" })}
        </button>
            ...
            <div className="space-y-2 rounded-xl border border-slate-600/50 bg-slate-800/40 p-3">
              <p className="text-xs font-medium text-slate-300">
                {t({
                  ...
                  ja: "直接入力（プライベートリポ含む）",
                  zh: "直接输入（含私有仓库）",
                  ru: "Прямой ввод (включая приватные репо)",
                })}
              </p>
              <div className="flex gap-2">
                ...
                >
                  {t({ ko: "", en: "Go", ja: "移動", zh: "前往", ru: "Перейти" })}
                </button>
              </div>
                ...
                {repos.map((repo) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">
                      {t({ ko: "", en: "Default", ja: "デフォルト", zh: "默认分支", ru: "По умолчанию" })}: {repo.default_branch}{" "}
        ...
        {step === "branch" && selectedRepo && (
          <div className="space-y-3">
            ...
            <h4 className="text-xs font-semibold text-slate-300">
              {t({ ko: "", en: "Select Branch", ja: "ブランチを選択", zh: "选择分支", ru: "Выбрать ветку" })}
            </h4>
            {branchError && (
              <div className="space-y-3">
                ...
                <div className="space-y-2 rounded-xl border border-amber-500/30 bg-amber-900/10 p-3">
                  <p className="text-xs font-medium text-amber-300">
                    {t({
                      ko: "Personal Access Token (PAT)",
                      en: "Authenticate with Personal Access Token (PAT)",
                      ja: "Personal Access Token (PAT) で認証",
                      zh: "使用 Personal Access Token (PAT) 认证",
                      ru: "Аутентификация с Personal Access Token (PAT)",
                    })}
                  </p>
                  <p className="text-[11px] text-slate-400">
                    {t({
                      ko: "GitHub Settings → Developer settings → Personal access tokens → Fine-grained tokens       .",
                      ...
                      ru: "Перейдите в GitHub Settings → Developer settings → Personal access tokens → Fine-grained tokens и создайте токен с доступом к этому репо.",
                    })}
                  </p>
                  <div className="flex gap-2">
                    <input
                    ...
                    >
                      {patLoading
                        ? t({ ko: "...", en: "Verifying...", ja: "確認中...", zh: "验证中...", ru: "Проверка..." })
                        : t({ ko: "", en: "Authenticate", ja: "認証", zh: "认证", ru: "Аутентифицировать" })}
                    </button>
                  </div>
                </div>
              </div>
            ...
            {branchesLoading ? (
              ...
              <p className="text-xs text-slate-500">
                {t({ ko: "", en: "No branches", ja: "ブランチなし", zh: "无分支", ru: "Нет веток" })}
              </p>
            ) : (
              <div className="space-y-1">
                {branches.map((branch) => (
        ...
        {step === "clone" && selectedRepo && selectedBranch && (
          <div className="space-y-4">
            ...
            <label className="block text-xs text-slate-400">
              {t({ ko: "", en: "Project Name", ja: "プロジェクト名", zh: "项目名称", ru: "Название проекта" })}
              <input
                type="text"
                value={projectName}
            ...
            <label className="block text-xs text-slate-400">
              {t({ ko: "", en: "Target Path", ja: "対象パス", zh: "目标路径", ru: "Целевой путь" })}
              <input
                type="text"
                value={targetPath}
            ...
            <label className="block text-xs text-slate-400">
              {t({
                ...
                ja: "コア目標（任意）",
                zh: "核心目标（可）",
                ru: "Основная цель (необязательно)",
              })}
              <textarea
                rows={3}
                value={coreGoal}
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">
                    {cloneStatus === "done"
                      ? t({ ko: "", en: "Complete", ja: "完了", zh: "完成", ru: "Завершено" })
                      : t({ ko: "...", en: "Cloning...", ja: "クローン中...", zh: "正在克隆...", ru: "Клонирование..." })}
                  </span>
            ...
            <div className="flex gap-2">
              ...
              >
                {creating
                  ? t({ ko: "...", en: "Importing...", ja: "インポート中...", zh: "正在导入...", ru: "Импорт..." })
                  : t({
                      ko: "GitHub",
                      en: "Import from GitHub",
                      ja: "GitHub からインポート",
                      zh: "从 GitHub 导入",
                      ru: "Импортировать с GitHub",
                    })}
              </button>
              <button
                type="button"
              ...
              >
                {t({ ko: "", en: "Back", ja: "戻る", zh: "返回", ru: "Назад" })}
              </button>
            </div>
          </div>
        )}
