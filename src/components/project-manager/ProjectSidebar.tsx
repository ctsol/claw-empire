    ...
    <aside className="flex w-full flex-col border-r border-slate-700 bg-slate-900/70 md:w-[330px]">
      ...
      <div className="border-b border-slate-700 px-4 py-3">
        ...
        <div className="mt-2 flex gap-2">
          ...
          >
            {t({ ko: "", en: "Search", ja: "検索", zh: "查询", ru: "Поиск" })}
          </button>
          <button
            type="button"
            onClick={startCreate}
          ...
          >
            {t({ ko: "", en: "New", ja: "新規", zh: "新建", ru: "Новый" })}
          </button>
          <button
            type="button"
            ...
            className="rounded-md bg-slate-700 px-2.5 py-1 text-xs text-slate-200 hover:bg-slate-600"
          >
            {t({ ko: "GitHub", en: "GitHub Import", ja: "GitHub インポート", zh: "GitHub 导入", ru: "Импорт GitHub" })}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loadingList ? (
          ...
          <div className="px-4 py-6 text-xs text-slate-500">
            {t({ ko: "", en: "No projects", ja: "プロジェクトなし", zh: "暂无项目", ru: "Нет проектов" })}
          </div>
        ) : (
