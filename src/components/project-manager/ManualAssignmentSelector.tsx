        ...
        <div className="mt-2 space-y-3">
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium text-slate-400">
              {t({ ko: "", en: "Assignment Mode", ja: "割り当てモード", zh: "分配模式", ru: "Режим назначения" })}
            </span>
            <div className="flex gap-1 rounded-lg border border-slate-700 bg-slate-800 p-0.5">
              ...
              >
                {t({ ko: "", en: "Auto", ja: "自動", zh: "自动", ru: "Авто" })}
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              >
                {t({ ko: "", en: "Manual", ja: "手動", zh: "手动", ru: "Вручную" })}
              </button>
            </div>
          </div>
            ...
            <div className="space-y-2 rounded-xl border border-slate-700 bg-slate-900/50 p-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-400">
                  {t({ ko: "", en: "Select Agents", ja: "エージェント選択", zh: "选择员工", ru: "Выбрать сотрудников" })}
                  <span className="ml-2 font-medium text-blue-400">
                    {selectedAgentIds.size}
                    {t({ ko: "", en: " selected", ja: "人", zh: "人", ru: " выбрано" })}
                  </span>
                </span>
                {departments.length > 0 && (
                  ...
                  >
                    <option value="all">{t({ ko: "", en: "All Depts", ja: "全部署", zh: "所有部门", ru: "Все отделы" })}</option>
        ...
        <div className="mt-2 rounded-lg border border-violet-500/20 bg-violet-600/10 px-3 py-2">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-violet-400">
              {t({ ko: "", en: "Manual Assignment", ja: "手動割り当て", zh: "手动分配", ru: "Ручное назначение" })}
            </span>
            <span className="text-xs text-slate-400">
              {detail?.assigned_agents?.length ?? 0}
              {t({ ko: "", en: " agents", ja: "人", zh: "人", ru: " сотрудников" })}
            </span>
          </div>
