      ...
      <section className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 sm:p-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-300">
            {t({ ko: "API", en: "API Providers", ja: "API プロバイダー", zh: "API 提供方", ru: "API Провайдеры" })}
          </h3>
          <div className="flex items-center gap-2">
            ...
            {!apiAddMode && (
              ...
              >
                + {t({ ko: "", en: "Add", ja: "追加", zh: "添加", ru: "Добавить" })}
              </button>
            )}
          </div>
        </div>
        ...
        {apiAddMode && (
          <div className="space-y-3 rounded-lg border border-blue-500/30 bg-slate-900/50 p-4">
            <h4 className="text-xs font-semibold uppercase text-blue-400">
              {apiEditingId
                ? t({ ko: "", en: "Edit Provider", ja: "プロバイダー編集", zh: "编辑提供方", ru: "Редактировать провайдера" })
                : t({
                    ko: "",
                    en: "Add New Provider",
                    ja: "新しいプロバイダーを追加",
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">
                {t({ ko: "", en: "Official Presets", ja: "公式プリセット", zh: "官方预设", ru: "Официальные пресеты" })}
              </label>
              <p className="mb-2 text-[11px] text-slate-500">
                {t({
                  ko: "OpenCode Go Bailian Coding Plan  .",
            ...
            {selectedOfficialPreset && (
              <div className="rounded-lg border border-blue-500/30 bg-blue-950/20 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  >
                    {t({ ko: "", en: "Open docs", ja: "ドキュメントを開く", zh: "打开文档", ru: "Открыть документацию" })}
                  </a>
                </div>
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">
                {t({ ko: "", en: "Generic Type", ja: "汎用タイプ", zh: "通用类型", ru: "Общий тип" })}
              </label>
              <p className="mb-2 text-[11px] text-slate-500">
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">
                {t({ ko: "", en: "Name", ja: "名前", zh: "名称", ru: "Название" })}
              </label>
              <input
                type="text"
                value={apiForm.name}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {apiSaving
                  ...
                  : apiEditingId
                    ? t({ ko: "", en: "Update", ja: "更新", zh: "更新", ru: "Обновить" })
                    : t({ ko: "", en: "Add", ja: "追加", zh: "添加", ru: "Добавить" })}
              </button>
              <button
                onClick={resetForm}
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex flex-shrink-0 items-center gap-1.5">
                      <button
                        ...
                        title={t({ ko: "", en: "Test Connection", ja: "接続テスト", zh: "连接测试", ru: "Проверить соединение" })}
                      >
                        {apiTesting === provider.id ? "..." : t({ ko: "", en: "Test", ja: "テスト", zh: "测试", ru: "Тест" })}
                      </button>
                      <button
                  ...
                  {provider.models_cache && provider.models_cache.length > 0 && (
                    <div className="mt-2">
                      ...
                      >
                        {isExpanded ? "-" : "+"}{" "}
                        {t({ ko: "", en: "Models", ja: "モデル一覧", zh: "模型列表", ru: "Модели" })} (
                          ...
                          <div className="max-h-48 overflow-y-auto rounded border border-slate-700/30 bg-slate-900/40 p-2">
                            {filteredModels.map((model) => (
                              ...
                              >
                                ...
                                >
                                  {t({ ko: "", en: "Assign", ja: "割当", zh: "分配", ru: "Назначить" })}
                                </button>
                              </div>
                            ))}
                            {filteredModels.length === 0 && (
                              <div className="text-[11px] text-slate-500 text-center py-2">
                                {t({ ko: "", en: "No results", ja: "結果なし", zh: "无结果", ru: "Нет результатов" })}
                              </div>
                            )}
                          </div>
                        </div>
