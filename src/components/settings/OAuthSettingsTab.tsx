    ...
    <section className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 sm:p-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider">
          {t({ ko: "OAuth", en: "OAuth Status", ja: "OAuth 認証状態", zh: "OAuth 认证状态", ru: "Статус OAuth" })}
        </h3>
      ...
      {oauthResult && (
        ...
        >
          <span>
            {oauthResult.error
              ? `${t({ ko: "OAuth", en: "OAuth connection failed", ja: "OAuth 接続失敗", zh: "OAuth 连接失败", ru: "Ошибка подключения OAuth" })}: ${oauthResult.error}`
              : `${OAUTH_INFO[oauthResult.provider || ""]?.label || oauthResult.provider} ${t({ ko: "!", en: "connected!", ja: "接続完了!", zh: "连接成功!", ru: "подключено!" })}`}
          </span>
          ...
          <span>
            {oauthStatus.storageReady
              ? t({
                  ...
                  ru: "Хранилище OAuth активно (ключ шифрования настроен)",
                })
              : t({
                  ko: "OAUTH_ENCRYPTION_SECRET",
                  ...
                  ru: "Переменная среды OAUTH_ENCRYPTION_SECRET не задана",
                })}
          </span>
        </div>
      )}
