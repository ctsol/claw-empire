...
export function categoryLabel(category: string, t: TFunction) {
  switch (category) {
    case "All":
      return t({ ko: "전체", en: "All", ja: "すべて", zh: "全部", ru: "Все" });
    case "Frontend":
      return t({ ko: "프론트엔드", en: "Frontend", ja: "フロントエンド", zh: "前端", ru: "Фронтенд" });
    case "Backend":
      return t({ ko: "백엔드", en: "Backend", ja: "バックエンド", zh: "后端", ru: "Бэкенд" });
    case "Design":
      return t({ ko: "디자인", en: "Design", ja: "デザイン", zh: "设计", ru: "Дизайн" });
    case "AI & Agent":
      return t({ ko: "AI & 에이전트", en: "AI & Agent", ja: "AI & エージェント", zh: "AI 与代理", ru: "ИИ и Агенты" });
    case "Marketing":
      return t({ ko: "마케팅", en: "Marketing", ja: "マーケティング", zh: "营销", ru: "Маркетинг" });
    case "Testing & QA":
      return t({ ko: "테스트 & QA", en: "Testing & QA", ja: "テスト & QA", zh: "测试与 QA", ru: "Тестирование и QA" });
    case "DevOps":
      return t({ ko: "데브옵스", en: "DevOps", ja: "DevOps", zh: "DevOps", ru: "DevOps" });
    case "Productivity":
      return t({ ko: "생산성", en: "Productivity", ja: "生産性", zh: "效率", ru: "Производительность" });
    case "Architecture":
      return t({ ko: "아키텍처", en: "Architecture", ja: "アーキテクチャ", zh: "架构", ru: "Архитектура" });
    case "Security":
      return t({ ko: "보안", en: "Security", ja: "セキュリティ", zh: "安全", ru: "Безопасность" });
    case "Other":
      return t({ ko: "기타", en: "Other", ja: "その他", zh: "其他", ru: "Прочее" });
    default:
      return category;
  }
}
...
export function localizeAuditStatus(status: string, t: TFunction): string {
  ...
  if (normalized === "pass") return t({ ko: "통과", en: "Pass", ja: "合格", zh: "通过", ru: "Пройдено" });
  if (normalized === "warn") return t({ ko: "경고", en: "Warn", ja: "警告", zh: "警告", ru: "Предупреждение" });
  if (normalized === "pending") return t({ ko: "대기", en: "Pending", ja: "保留", zh: "待处리", ru: "Ожидание" });
  if (normalized === "fail") return t({ ko: "실패", en: "Fail", ja: "失敗", zh: "失败", ru: "Провалено" });
  return status;
}
...
export function roleLabel(role: AgentRole, t: TFunction): string {
  if (role === "team_leader") return t({ ko: "팀장", en: "Team Lead", ja: "チームリード", zh: "团队负责人", ru: "Руководитель" });
  if (role === "senior") return t({ ko: "시니어", en: "Senior", ja: "シニア", zh: "资深", ru: "Старший" });
  if (role === "junior") return t({ ko: "주니어", en: "Junior", ja: "ジュニア", zh: "初级", ru: "Младший" });
  return t({ ko: "인턴", en: "Intern", ja: "インターン", zh: "实习生", ru: "Стажёр" });
}
...
export function learningStatusLabel(status: SkillLearnJob["status"] | null, t: TFunction): string {
  if (status === "queued") return t({ ko: "대기중", en: "Queued", ja: "待機中", zh: "排队中", ru: "В очереди" });
  if (status === "running") return t({ ko: "학습중", en: "Running", ja: "学習中", zh: "学习中", ru: "Выполняется" });
  if (status === "succeeded") return t({ ko: "완료", en: "Succeeded", ja: "完了", zh: "完成", ru: "Завершено" });
  if (status === "failed") return t({ ko: "실패", en: "Failed", ja: "失敗", zh: "失败", ru: "Неудача" });
  return "-";
}
