...
export function categoryLabel(category: string, t: TFunction) {
  switch (category) {
    case "All":
      return t({ ko: "", en: "All", ja: "すべて", zh: "全部", ru: "Все" });
    case "Frontend":
      return t({ ko: "", en: "Frontend", ja: "フロントエンド", zh: "前端", ru: "Фронтенд" });
    case "Backend":
      return t({ ko: "", en: "Backend", ja: "バックエンド", zh: "后端", ru: "Бэкенд" });
    case "Design":
      return t({ ko: "", en: "Design", ja: "デザイン", zh: "设计", ru: "Дизайн" });
    case "AI & Agent":
      return t({ ko: "AI &", en: "AI & Agent", ja: "AI & エージェント", zh: "AI 与代理", ru: "ИИ и Агенты" });
    case "Marketing":
      return t({ ko: "", en: "Marketing", ja: "マーケティング", zh: "营销", ru: "Маркетинг" });
    case "Testing & QA":
      return t({ ko: "& QA", en: "Testing & QA", ja: "テスト & QA", zh: "测试与 QA", ru: "Тестирование и QA" });
    case "DevOps":
      return t({ ko: "", en: "DevOps", ja: "DevOps", zh: "DevOps", ru: "DevOps" });
    case "Productivity":
      return t({ ko: "", en: "Productivity", ja: "生産性", zh: "效率", ru: "Производительность" });
    case "Architecture":
      return t({ ko: "", en: "Architecture", ja: "アーキテクチャ", zh: "架构", ru: "Архитектура" });
    case "Security":
      return t({ ko: "", en: "Security", ja: "セキュリティ", zh: "安全", ru: "Безопасность" });
    case "Other":
      return t({ ko: "", en: "Other", ja: "その他", zh: "其他", ru: "Прочее" });
    default:
      return category;
  }
}
...
export function localizeAuditStatus(status: string, t: TFunction): string {
  ...
  if (normalized === "pass") return t({ ko: "", en: "Pass", ja: "合格", zh: "通过", ru: "Пройдено" });
  if (normalized === "warn") return t({ ko: "", en: "Warn", ja: "警告", zh: "警告", ru: "Предупреждение" });
  if (normalized === "pending") return t({ ko: "", en: "Pending", ja: "保留", zh: "待处", ru: "Ожидание" });
  if (normalized === "fail") return t({ ko: "", en: "Fail", ja: "失敗", zh: "失败", ru: "Провалено" });
  return status;
}
...
export function roleLabel(role: AgentRole, t: TFunction): string {
  if (role === "team_leader") return t({ ko: "", en: "Team Lead", ja: "チームリード", zh: "团队负责人", ru: "Руководитель" });
  if (role === "senior") return t({ ko: "", en: "Senior", ja: "シニア", zh: "资深", ru: "Старший" });
  if (role === "junior") return t({ ko: "", en: "Junior", ja: "ジュニア", zh: "初级", ru: "Младший" });
  return t({ ko: "", en: "Intern", ja: "インターン", zh: "实习生", ru: "Стажёр" });
}
...
export function learningStatusLabel(status: SkillLearnJob["status"] | null, t: TFunction): string {
  if (status === "queued") return t({ ko: "", en: "Queued", ja: "待機中", zh: "排队中", ru: "В очереди" });
  if (status === "running") return t({ ko: "", en: "Running", ja: "学習中", zh: "学习中", ru: "Выполняется" });
  if (status === "succeeded") return t({ ko: "", en: "Succeeded", ja: "完了", zh: "完成", ru: "Завершено" });
  if (status === "failed") return t({ ko: "", en: "Failed", ja: "失敗", zh: "失败", ru: "Неудача" });
  return "-";
}
