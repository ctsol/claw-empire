    ...
    <div className="skills-learn-modal fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4">
      <div className="skills-learn-modal-card w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900/95 shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-700/60 px-5 py-4">
          ...
          >
            {learnInProgress
              ? t({ ko: "", en: "Running", ja: "実行中", zh: "进行中", ru: "Выполняется" })
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4 max-h-[calc(90vh-72px)]">
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/8 px-3 py-2">
            <div className="text-[11px] text-emerald-200">
              {t({ ko: "", en: "Install command", ja: "実行コマンド", zh: "执命", ru: "Команда установки" })}
            </div>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-[11px] text-slate-500">
              ...
              {t({ ko: "", en: " selected", ja: "名を選択", zh: "已", ru: " выбрано" })}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {representatives.map((row) => {
              ...
              const displayName = row.agent
                ...
                : t({ ko: "", en: "No assigned member", ja: "担メンバーなし", zh: "暂无成员", ru: "Нет назначенного участника" });
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    >
                      ...
                      {isHitAnimating && (
                        <span className="unlearn-hit-text">
                          {t({ ko: "~", en: "Bonk!", ja: "ゴン!", zh: "咣~", ru: "Бонк!" })}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {row.agent
                          ...
                          : t({ ko: "", en: "Unavailable", ja: "利用不可", zh: "不可用", ru: "Недоступно" })}
                      </div>
                    </div>
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1.5">
                      ...
                      >
                        {isAlreadyLearned
                          ? t({ ko: "", en: "Learned", ja: "習済み", zh: "已学习", ru: "Изучено" })
                          : isSelected
                            ? t({ ko: "", en: "Selected", ja: "選択", zh: "已", ru: "Выбрано" })
                            : t({ ko: "", en: "Idle", ja: "待機", zh: "待命", ru: "Ожидание" })}
                      </div>
                      {isAlreadyLearned && (
                        <button
                          type="button"
                        ...
                        >
                          {isUnlearning
                            ? t({ ko: "...", en: "Unlearning...", ja: "取消中...", zh: "取消中...", ru: "Отмена обучения..." })
                            : t({ ko: "", en: "Unlearn", ja: "習", zh: "消", ru: "Забыть" })}
                        </button>
                      )}
                    </div>
                  </div>
          ...
          <div className="rounded-xl border border-slate-700/70 bg-slate-800/55 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="text-slate-300">
                {t({ ko: "", en: "Job status", ja: "ジョブ状態", zh: "任务状态", ru: "Статус задания" })}:{" "}
                <span
              ...
              <div className="mt-2 rounded-lg border border-slate-700 bg-slate-900/70 p-2 font-mono text-[10px] text-slate-300 max-h-32 overflow-y-auto space-y-1">
                ...
                {learnJob.logTail.length > 0 ? (
                  ...
                  <div className="text-slate-600">
                    {t({ ko: "", en: "No logs yet", ja: "ログはまだありません", zh: "暂无日志", ru: "Логов пока нет" })}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {learnSubmitting || learnInProgress
                ? t({ ko: "...", en: "Learning...", ja: "学習中...", zh: "習中...", ru: "Обучение..." })
                : t({ ko: "", en: "Start Learning", ja: "学習開始", zh: "开始", ru: "Начать обучение" })}
            </button>
          </div>
        </div>
      </div>
