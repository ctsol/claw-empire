...
\`\`\``,
    ...
    const prompt = buildTaskExecutionPrompt(
      [
        ...
        conversationCtx,
        `\n---`,
        `Role: ${roleLabel} (${agent.department_name || "Unassigned"})`,
        agent.personality ? `Work style: ${agent.personality}` : "",
        deptConstraint,
    ...
    if (provider === "api") {
      ...
      const worktreeNote = pickL(
        l(
          ...
          [` (изолированная ветка: climpire/${id.slice(0, 8)})`],
        ),
        taskLang,
      );
      notifyCeo(
        pickL(
          l(
            ...
            [`${assigneeName} приступил к работе над '${task.title}'.${worktreeNote}`],
          ),
          taskLang,
        ),
        id,
